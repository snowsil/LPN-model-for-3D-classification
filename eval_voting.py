...
class PointcloudScale(object):  # input random scaling
    ...
    def __call__(self, pc):
        ...
        for i in range(bsize):
            ...
            scales = torch.from_numpy(xyz).float().to(pc.device)
...
def test(args, io):
    ...
    device = torch.device("cuda:%d"%args.gpus[0])
    NUM_PEPEAT = 300
    NUM_VOTE = 10
    ...
    if args.arch == 'dgcnn':
        ...
        model = PAConv(args).to(device)
    elif args.arch == 'interpcnn2':
        from model.interpcnn2 import InterpCNN2
        model = InterpCNN2(args).to(device)
    else:
    ...
    for i in range(NUM_PEPEAT):
        ...
        pbar = tqdm(test_loader, desc=f'repeat: {i}')
        for data, label in pbar:
            ...
            for v in range(NUM_VOTE):
                ...
                if v > 0:
                    new_data.data = pointscale(new_data)
                with torch.no_grad():
...
if __name__ == "__main__":
    ...
    assert torch.cuda.is_available(), "cuda training is required."
